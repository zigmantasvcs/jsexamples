...
function getWordsLengthAverageV2(words) {
    return words.reduce((result, currentWord) => result += currentWord.length, 0) / words.length;
}
...
function returnWordOpositeV1(word){
    let opositeWord = "";
...
function returnWordOpositeV2(word){
...
function processPrices(){
    fetch("http://zigmantas.codingschoolprojektai.lt/api/?data=ps5")
        ...
        .then(data => getTopLowestPricesV2(data))
        .then(topPrices => console.log(topPrices))
}

// version 1
function getTopLowestPricesV1(prices){
    return prices
        .sort((a, b) => {
...
// version 2
function getTopLowestPricesV2(prices){
    let sortedPrices = prices.sort((a, b) => { return a.price - b.price; });
